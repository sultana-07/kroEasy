...
export default function LabourCard({ labour, userId, searchedCity }) {
  const { t } = useLanguage();
  const navigate = useNavigate();
  const user = labour.userId;
  const rating = labour.rating || 0;

  // If a city was searched and worker serves it via serviceCities, show that
  const primaryCity = user?.city || labour.city;
  const servesSearched = searchedCity && labour.serviceCities?.includes(searchedCity) && primaryCity !== searchedCity;
  const displayCity = searchedCity || primaryCity;

  const goToProfile = () => navigate(`/profile/labour/${labour._id}`);
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
        ...
        {labour.profileImage ? (
          <img src={labour.profileImage} alt={user?.name}
            style={{ width: '52px', height: '52px', borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid #EEF2FF' }} />
        ) : (
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={{ fontSize: '16px', fontWeight: '700', color: '#0F172A' }}>{user?.name} <span style={{ fontSize: '10px', verticalAlign: 'middle', opacity: 0.5 }}>›</span></div>
              <div style={{ fontSize: '12px', color: '#64748B', display: 'flex', alignItems: 'center', gap: '5px' }}>
                🏙️ {displayCity}
                {servesSearched && (
                  <span style={{ fontSize: '10px', background: '#ECFDF5', color: '#16A34A', border: '1px solid #BBF7D0', borderRadius: '10px', padding: '1px 7px', fontWeight: '700' }}>
                    ✓ Serves here
                  </span>
                )}
              </div>
